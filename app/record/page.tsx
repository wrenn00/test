...
export default async function Page({ searchParams }: { searchParams: Promise<{ d?: string; edit?: string; plan?: string }> }) {
  const { d, edit, plan } = await searchParams;
  ...
  return (
    <main className="min-h-screen flex items-center justify-center py-10 px-4">
      <div className="relative w-[375px] h-[812px] bg-bg rounded-[28px] shadow-[0_8px_40px_rgba(0,0,0,0.12)] overflow-hidden">
        <RecordScreen initialDay={day} edit={edit === "1"} initialPlan={plan} />
      </div>
    </main>
  );
}
